...
function openDialog(update = false, id = -1) {
    ...
    if (id == -1) {
        ...
        document.getElementById("nim").value = dataUser[id]["nim"];
...
function submit() {
    ...
    if (!modalUpdate) {
        //CREATE
        var endpoint = `${config.api}createNewUser`;
        var meth = "POST";
    } else {
        //UPDATE
        var endpoint = `${config.api}updateUserProfile`;
...
form.addEventListener("click", () => {
    ...
    if (nim.value.trim() === "") {
        setErrorMsg(nim, "Please insert NIM correctly");
        validate["nim"] = false;
    } else if (nim.value.trim().length < 3) {
        setErrorMsg(nim, "Please input minimum 3 character");
        validate["nim"] = false;
    } else {
        setSuccessMsg(nim);
        validate["nim"] = true;
