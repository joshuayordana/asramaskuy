...
function updateTable(search = "", date_in = null, date_out = null) {
  ...
  if (search != "" || date_in != null || date_out != null) {
    ...
    for (let i = 0; i < temp.length; i++) {
      ...
      tr.innerHTML += `<td>${dateFormat(temp[i]["tanggal_transaksi"].slice(0, 10))} ${takeTime(dataUser[i]["tanggal_transaksi"])}</td>`;
    ...
    for (let i = 0; i < dataUser.length; i++) {
      const tr = document.createElement("tr");
      tr.setAttribute("id", `guest-${i}`);
      console.log(dataUser[i]["tanggal_transaksi"]);
      tr.innerHTML = `<td>${dataUser[i]["nama_transaksi"]}</td>`;
      tr.innerHTML += `<td>${dateFormat(takeDate(dataUser[i]["tanggal_transaksi"]))} ${takeTime(dataUser[i]["tanggal_transaksi"])}</td>`;
